...
`);
            ...
            const userServiceExports = result.exports.filter((e) => e.name === "UserService");
...
`);
            ...
            expect(result.functions.map((f) => f.name).sort()).toEqual(["GetUsers", "Log", "UserService"].sort());
            ...
            expect(userService.methods.sort()).toEqual(["GetUsers", "Log", "UserService"].sort());
            expect(userService.properties.sort()).toEqual(["MaxRetries", "_name"].sort());
            const repository = result.classes.find((c) => c.name === "IRepository");
            ...
            expect(getUsersCalls.some((e) => e.callee === "FetchFromDb")).toBe(true);
            const logCalls = calls.filter((e) => e.caller === "Log");
            expect(logCalls.some((e) => e.callee === "Console.WriteLine")).toBe(true);
            tree.delete();
            parser.delete();
        });
    });
...
//# sourceMappingURL=csharp-extractor.test.js.map