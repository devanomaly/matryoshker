...
`);
            ...
            expect(result.functions.map((f) => f.name).sort()).toEqual(["UserService", "getUsers", "log"].sort());
            ...
            expect(userService.methods.sort()).toEqual(["UserService", "getUsers", "log"].sort());
            expect(userService.properties.sort()).toEqual(["name", "maxRetries"].sort());
            ...
            expect(logCalls.some((e) => e.callee === "System.out.println")).toBe(true);
            tree.delete();
            parser.delete();
        });
    });
...
//# sourceMappingURL=java-extractor.test.js.map