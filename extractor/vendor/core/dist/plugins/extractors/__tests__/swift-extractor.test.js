...
beforeAll(async () => {
    ...
    await Parser.init();
    const wasmPath = require.resolve("@understand-anything/tree-sitter-swift-wasm/tree-sitter-swift.wasm");
...
describe("SwiftExtractor", () => {
    ...
    describe("extractStructure - functions", () => {
        ...
        it("ignores async and throws while preserving declared return type", () => {
            withAnalysis(`func fetch<T: Decodable>(_ type: T.Type) async throws -> T { fatalError() }\n`, (result) => {
...
`, (result) => {
                ...
                expect(result.classes[0].properties).toEqual(expect.arrayContaining(["id", "name", "kind"]));
...
`, (result) => {
        ...
        it("extracts protocol associated types, properties, functions, and init requirements", () => {
            withAnalysis(`protocol Repository {
  associatedtype Item
  var id: String { get }
...
`, (result) => {
        ...
        it("does not treat local declarations in property initializers as properties", () => {
            withAnalysis(`public struct Store {
  public let value = { let local = 1; return local }()
}
`, (result) => {
                expect(result.classes[0].properties).toEqual(["value"]);
                expect(result.exports.map((e) => e.name)).toEqual(expect.arrayContaining(["Store", "value"]));
    ...
    describe("extractStructure - visibility and exports", () => {
        ...
        it("exports public, open, and package declarations", () => {
            withAnalysis(`public func api() {}\nopen class Base {}\npackage struct ModuleOnly {}\n`, (result) => {
                expect(result.exports.map((e) => e.name)).toEqual(expect.arrayContaining(["api", "Base", "ModuleOnly"]));
            });
        });
        ...
        it("exports visible members and excludes private members", () => {
            withAnalysis(`public class Service {
  public func start() {}
  private func debug() {}
}
`, (result) => {
        ...
        it("exports public private(set) properties", () => {
            withAnalysis(`public struct Counter { public private(set) var value = 0 }\n`, (result) => {
                expect(result.exports.map((e) => e.name)).toEqual(expect.arrayContaining(["Counter", "value"]));
            });
        });
    ...
    describe("extractCallGraph", () => {
        ...
        it("attributes calls inside closures to the enclosing function", () => {
            withCalls(`func build(_ items: [Item]) { items.map { transform($0) }.filter { isValid($0) } }\n`, (calls) => {
                expect(calls.map((c) => c.callee)).toEqual(expect.arrayContaining(["map", "transform", "filter", "isValid"]));
...
`;
            ...
            expect(calls.map((c) => c.callee)).toEqual(expect.arrayContaining(["WindowGroup", "ContentView"]));
        });
    });
});
//# sourceMappingURL=swift-extractor.test.js.map