...
beforeAll(async () => {
    ...
    await Parser.init();
    const wasmPath = require.resolve("@tree-sitter-grammars/tree-sitter-kotlin/tree-sitter-kotlin.wasm");
...
`);
            ...
            expect(result.classes[0].properties).toEqual(expect.arrayContaining(["bar", "baz"]));
...
`);
            ...
            expect(result.classes[0].methods).toEqual(expect.arrayContaining(["greet", "farewell"]));
            tree.delete();
            parser.delete();
        });
    });
...
`);
            ...
            expect(result.classes[0].methods).toEqual(expect.arrayContaining(["info", "warn"]));
            tree.delete();
            parser.delete();
        });
    });
...
//# sourceMappingURL=kotlin-extractor.test.js.map