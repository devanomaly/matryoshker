...
`);
        ...
        it("extracts curried and using-clause parameter lists", () => {
            const { tree, parser, root } = parse(`def run(a: Int)(b: String)(using ec: scala.concurrent.ExecutionContext): Unit = ()
`);
            const result = extractor.extractStructure(root);
            expect(result.functions).toHaveLength(1);
...
`);
        ...
        it("treats only val/var constructor params of a regular class as properties", () => {
            const { tree, parser, root } = parse(`class Service(val name: String, dep: Int, var counter: Long)
`);
            const result = extractor.extractStructure(root);
            expect(result.classes).toHaveLength(1);
...
`);
            ...
            expect(result.exports.map((e) => e.name)).toEqual(expect.arrayContaining(["syntax", "shout"]));
            tree.delete();
            parser.delete();
        });
...
`);
            ...
            expect(result.exports.map((e) => e.name)).toEqual(expect.arrayContaining(["Foo", "run", "Bar"]));
            tree.delete();
            parser.delete();
        });
...
`);
            ...
            expect(result.exports.map((e) => e.name)).toEqual(expect.arrayContaining(["defaultTimeout", "helper", "syntax"]));
            tree.delete();
            parser.delete();
        });
...
`);
            ...
            expect(entries).toContainEqual(expect.objectContaining({ caller: "run", callee: "helper" }));
            expect(entries).toContainEqual(expect.objectContaining({ caller: "run", callee: "getUser" }));
            tree.delete();
            parser.delete();
        });
...
`);
            const entries = extractor.extractCallGraph(root);
            expect(entries).toContainEqual(expect.objectContaining({ caller: "caller", callee: "map" }));
            expect(entries).toContainEqual(expect.objectContaining({ caller: "caller", callee: "Foo" }));
            tree.delete();
            parser.delete();
        });
...
//# sourceMappingURL=scala-extractor.test.js.map