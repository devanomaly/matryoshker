...
function runGit(projectDir, args) {
    return new Promise((resolve, reject) => {
        execFile("git", args, {
            ...
            if (error) {
                reject(new GitCommandError(typeof error.code === "number" ? error.code : null, error.killed === true && error.signal !== null));
                return;
            }
...
async function createProjectGitSnapshot(projectDir) {
    const repoRoot = parseScalar(await runGit(projectDir, ["rev-parse", "--show-toplevel"]));
    const headCommitHash = parseScalar(await runGit(projectDir, ["rev-parse", "HEAD"]));
    ...
    return {
        ...
        repoRoot,
        headCommitHash,
        dirtyFiles: uniqueSortedPaths(parseNulDelimitedPaths(staged), parseNulDelimitedPaths(unstaged), parseNulDelimitedPaths(untracked)),
    };
}
...
async function evaluateGraphFreshness(snapshot, input, requestedGraphCommitHash) {
    let graphCommitHash;
    try {
        graphCommitHash = parseScalar(await runGit(snapshot.projectDir, [
            "rev-parse",
            "--verify",
            "--end-of-options",
    ...
    let committedFiles;
    try {
        committedFiles = parseNulDelimitedPaths(await runGit(snapshot.projectDir, [
            "diff",
            "--name-only",
            "-z",
            "--relative",
    ...
    try {
        const [countsOutput, graphIsAncestor, headIsAncestor] = await Promise.all([
            ...
            isAncestor(snapshot.projectDir, graphCommitHash, snapshot.headCommitHash),
            isAncestor(snapshot.projectDir, snapshot.headCommitHash, graphCommitHash),
        ]);
        ...
        const relation = graphIsAncestor
            ? "behind"
            : headIsAncestor
                ? "ahead"
                : "diverged";
        const changedFiles = uniqueSortedPaths(committedFiles, snapshot.dirtyFiles);
        return {
            status: "stale",
            relation,
            graphCommitHash,
 ...
 */
export function getChangedFiles(projectDir, lastCommitHash) {
    try {
 ...
 */
export function isStale(projectDir, lastCommitHash) {
 ...
 */
export async function getGraphFreshnessBatch(projectDir, inputs) {
    ...
    await Promise.all(comparableEntries.map(async ([key, input, graphCommitHash]) => {
        results[key] = await evaluateGraphFreshness(snapshot, input, graphCommitHash);
    }));
    return results;
}
/**
 ...
 */
export async function getGraphFreshness(projectDir, input) {
 ...
 */
export function mergeGraphUpdate(existingGraph, changedFilePaths, newNodes, newEdges, newCommitHash) {
    const changedSet = new Set(changedFilePaths);
    // Collect IDs of nodes that belong to changed files (will be removed)
    const removedNodeIds = new Set(existingGraph.nodes
    ...
    const retainedEdges = existingGraph.edges.filter((edge) => !removedNodeIds.has(edge.source) && !removedNodeIds.has(edge.target));
    return {
        ...existingGraph,
        project: {
