...
export function extractTokens(doc, styles, structuralNodes, fileKey) {
    const nodes = [];
    const edges = [];
    const tokenByStyleKey = new Map();
    ...
    for (const s of styles.meta?.styles ?? []) {
        ...
        if (!tokenByStyleKey.has(s.key))
            tokenByStyleKey.set(s.key, id);
    ...
    const graphIdByFigmaId = new Map();
    for (const n of structuralNodes) {
        if (n.figmaMeta?.nodeId)
            graphIdByFigmaId.set(n.figmaMeta.nodeId, n.id);
    }
    const usesSeen = new Set();
    function walk(n, nearestConsumerId) {
