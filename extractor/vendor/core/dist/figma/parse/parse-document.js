function mkNode(type, figmaId, name, figmaMeta) {
    return {
        id: `${type}:${figmaId}`,
        type,
        name,
...
export function parseDocument(doc, fileKey) {
    const nodes = [];
    const edges = [];
    const seen = new Set();
    const add = (n) => { if (!seen.has(n.id)) {
        seen.add(n.id);
        nodes.push(n);
    } };
    const link = (source, target, type, weight) => edges.push({ source, target, type, direction: "forward", weight });
    // Deep-read a screen subtree to find instances (shallow node set, but deep read).
    function collectInstances(n, screenId) {
        for (const child of n.children ?? []) {
            if (child.type === "INSTANCE") {
                ...
                if (child.componentId)
                    link(inst.id, `component:${child.componentId}`, "instance_of", 0.8);
            }
            if (child.children)
                collectInstances(child, screenId);
        }
    }
    function handlePageChild(child, pageId) {
        switch (child.type) {
            case "FRAME": {
            ...
            case "SECTION": {
                for (const sub of child.children ?? [])
                    handlePageChild(sub, pageId); // flatten sections in v1
                break;
            }
            default:
