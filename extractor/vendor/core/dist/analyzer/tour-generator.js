 ...
 */
export function buildTourGenerationPrompt(graph) {
    ...
    const layerList = layers.length > 0
        ? layers
            .map((l) => `  - ${l.name}: ${l.description} (nodes: ${l.nodeIds.join(", ")})`)
            .join("\n")
        : "  (no layers detected)";
 ...
 */
export function parseTourGenerationResponse(response) {
    ...
    try {
        ...
        for (const item of parsed.steps) {
            ...
            if (typeof item.description !== "string" || item.description.length === 0)
                continue;
            if (!Array.isArray(item.nodeIds) || item.nodeIds.length === 0)
                continue;
            const step = {
                order: item.order,
                title: item.title,
                description: item.description,
                nodeIds: item.nodeIds.filter((id) => typeof id === "string"),
            };
 ...
 */
export function generateHeuristicTour(graph) {
    ...
    if (layers.length > 0) {
        ...
        const layeredNodeIds = new Set(layers.flatMap((l) => l.nodeIds));
        const unlayeredNodes = topoOrder.filter((id) => !layeredNodeIds.has(id));
